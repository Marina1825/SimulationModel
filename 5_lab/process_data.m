function [L] = process_data(data_complex, distance)%, noise_power, rssi)%process_data(data_raw)
    ...
    % Расчет RSRP
    %rsrp = mean(abs(data_comple).^2); % Средняя мощность принимаемого сигнала

    % Расчет SINR
    %sinr = rsrp / noise_power; % Отношение мощности сигнала к мощности шума и помех

    % Расчет RSRQ
    %rsrq = rsrp / rssi; % Отношение мощности опорного сигнала к общей мощности принимаемого сигнала
